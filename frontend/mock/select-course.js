...
export default [
  // find
  {
    url: '/vue-element-admin/select-course/find',
    type: 'post',
    response: config => {
  ...
  // create
  {
    url: '/vue-element-admin/select-course/create',
    type: 'post',
    response: config => {
      const { data } = config.body
      ...
      for(let i = 0; i < selectCourse.length; ++i){
        if(selectCourse[i].userId === data.userId && selectCourse[i].courseId === data.courseId){
          exists = true
          break;
        }
      }
  ...
  // remove
  {
    url: '/vue-element-admin/select-course/remove',
    type: 'delete',
    response: config => {
      const { data } = config.body
      if(!data.userId || !data.courseId){
      ...
      if(index >= 0){
        selectCourse[index] = selectCourse[0]
        selectCourse.pop()
        return {
          code: 20000,
          data: 'success'
      ...
      return {
        code: 456778,
        data: 'error: there is no course with course id' + data.courseId + ' learned by user id ' + data.userId
      }
    }
  }
]
