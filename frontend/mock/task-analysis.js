const taskAnalysis = require('./db').taskAnalysis

export default [
  // find
  {
    url: '/vue-element-admin/task-analysis/find',
    type: 'post',
    response: config => {
  ...
  // create
  {
    url: '/vue-element-admin/task-analysis/create',
    type: 'post',
    response: config => {
      const { data } = config.body
      if(!data.taskId){
  ...
  // update
  {
    url: '/vue-element-admin/task-analysis/update',
    type: 'post',
    response: config => {
      const { data } = config.body
      if(!data.taskId){
      ...
      let index = -1
      for(let i = 0; i < taskAnalysis.length; ++i){
        if(taskAnalysis[i].taskId === data.taskId){
          index = i
          break;
        }
      }
      ...
      return {
        code: 456778,
        data: 'error: there is no task analysis corresponding to id ' + data.taskId
      }
    }
  },
  ...
  // remove
  {
    ...
    response: config => {
      ...
      if(index >= 0){
        taskAnalysis[index] = taskAnalysis[0]
        task.pop()
        return {
          code: 20000,
          data: 'success'
